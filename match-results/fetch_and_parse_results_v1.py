    ...
    for table in tables:
        ...
        for row in rows:
            ...
            if parsed_body_titles_cols is not None:
                ...
                #if jornada_header_info["left_team_name"] == jornada_header_info["abc_team_name"]:
                if jornada_header_info["left_team_name"] == parsed_body_titles_cols["abc_team"]:
                    visitor_on_the_left = False
                else:
                    visitor_on_the_left = True
            ...
            match = parse_jornada_body_from_td_cols(cols, jornada_number, jornada_header_info, matches_id_seen, visitor_on_the_left)
...
def parse_for_header_jornada_info(cols):
    ...
    if teams_info is None:
        return None
    response["left_team_id"] = teams_info["left_team"]["id"]
    response["left_team_name"] = teams_info["left_team"]["name"]
    response["right_team_id"] = teams_info["right_team"]["id"]
    response["right_team_name"] = teams_info["right_team"]["name"]
    return response

def parse_for_body_titles_cols(cols):
    if len(cols) > 10 and cols[5].get_text() == 'J1' and cols[6].get_text() == 'J2' and cols[7].get_text() == 'J3':
        return {"left_team": cols[2].get_text(), "right_team": cols[4].get_text(), "abc_team": cols[2].get_text(), "xyz_team": cols[4].get_text()}
    return None

def parse_jornada_body_from_td_cols(cols, jornada_number, jornada_header_info, matches_id_seen, is_visitor_on_the_left):
    ...
    if player1_text.startswith("Lic:"):
        ...
        single_players_infos = {"left": info_player1, "right": info_player2}
    ...
    doubles_players_names = {"left": info_player12 , "right": info_player22 }
    ...
    match = {
        ...
        "Local team id": (get_local_team_id(jornada_header_info, is_visitor_on_the_left) if jornada_header_info is not None else ""),
        "Local team name": (get_local_team_name(jornada_header_info, is_visitor_on_the_left) if jornada_header_info is not None else ""),
        "Visitor team id": (get_visitor_team_id(jornada_header_info, is_visitor_on_the_left) if jornada_header_info is not None else ""),
        "Visitor team name": (get_visitor_team_name(jornada_header_info, is_visitor_on_the_left) if jornada_header_info is not None else ""),
        "Player 1 Name": get_local_player_name(single_players_infos, is_visitor_on_the_left) if single_players_infos else '',
        "Player 1 License": get_local_player_license(single_players_infos, is_visitor_on_the_left) if single_players_infos else '',
        "Player 1 Letter": contender_letter_1 if contender_letter_1 else '',
        #"Player 1 Rank": info_player1[2] if info_player1 else '',
        "Player 2 Name": get_visitor_player_name(single_players_infos, is_visitor_on_the_left) if single_players_infos else '',
        "Player 2 License": get_visitor_player_license(single_players_infos, is_visitor_on_the_left) if single_players_infos else '',
...
def parse_jornada_teams_header(header_cols):
    left_col_element = header_cols[1]
    left_team_name = left_col_element.get_text()
    href_element = left_col_element.find("a").get("href")
    parsed_url = urlparse(href_element)
    params = parse_qs(parsed_url.query)
    left_team_id = params["equipo"][0]

    right_col_element = header_cols[4]
    right_team_name = right_col_element.get_text()
    href_element = right_col_element.find("a").get("href")
    parsed_url = urlparse(href_element)
    params = parse_qs(parsed_url.query)
    right_team_id = params["equipo"][0]

    return {"right_team": {"name": right_team_name, "id": right_team_id}, "left_team": {"name": left_team_name, "id": left_team_id}}
...
def get_local_team_id(jornada_header_info, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return jornada_header_info["right_team_id"]
    else:
        return jornada_header_info["left_team_id"]

def get_local_team_name(jornada_header_info, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return jornada_header_info["right_team_name"]
    else:
        return jornada_header_info["left_team_name"]

def get_visitor_team_id(jornada_header_info, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return jornada_header_info["left_team_id"]
    else:
        return jornada_header_info["right_team_id"]

def get_visitor_team_name(jornada_header_info, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return jornada_header_info["left_team_name"]
    else:
        return jornada_header_info["right_team_name"]

def get_local_player_name(single_players_infos, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return single_players_infos["right"][0]
    else:
        return single_players_infos["left"][0]

def get_visitor_player_name(single_players_infos, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return single_players_infos["left"][0]
    else:
        return single_players_infos["right"][0]

def get_local_player_license(single_players_infos, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return single_players_infos["right"][1]
    else:
        return single_players_infos["left"][1]

def get_visitor_player_license(single_players_infos, is_visitor_on_the_left):
    if is_visitor_on_the_left:
        return single_players_infos["left"][1]
    else:
        return single_players_infos["right"][1]

def get_local_doubles_player_id(jornada_header_info, is_visitor_on_the_left):
    return

def get_local_doubles_player_name(jornada_header_info, is_visitor_on_the_left):
    return
